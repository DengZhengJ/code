...
/*1、确定主设备号，也可以让内核分配*/
static int major = 0;   //设备号为0 系统自动分配  static定义全局变量，作用于定义它的文件
static struct class *led_class;
struct led_operations *p_led_opr; //定义一个指向led_operations结构体的指针p_led_opr


/*3、内核实现对应的 drv_open drv_read drv_write等函数，填入file_operations结构体*/
...
static int __init led_init(void)
{
	...
	printk("%s %s line %d\n", __FILE__, __FUNCTION__, __LINE__);	
	major = register_chrdev(0, "100ask_led", &led_drv);  /*注册设备驱动 /proc/devices */

	/*建立了逻辑设备类*/
	led_class = class_create(THIS_MODULE, "100sak_led_class");	/*会在/sys/class/100sak_led_class 下创建访问节点*/
...
module_init(led_init);   	//修饰为入口函数 注册模块加载函数 
